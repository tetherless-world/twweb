function loadPage(year, page) {
	...
	$.ajax({url: window.twsparql,
		data: {"year": year,
		       ...
		       "query": "person-publications-paginate-footer.rq",
		       "xslt": "publications-paginate-footer.xsl"},
		success: function(data,textStatus,xhr) {
		    window.data = data;
		    var elem = $("*[class=\"publications-paginate-footer\"]",data);
		    window.elem = elem;
		    var node;
		    if(!$.browser.msie) {
			node = document.importNode(elem[0],true);
			window.node = node;
			$(".publications-paginate-footer").replaceWith(node);
		    }
		    else {
			$(".publications-paginate-footer").replaceWith(/(<div.*>.*<\/div>)/.exec(xhr.responseText)[0]);
			node = $(".publications-paginate-footer");
		    }
    ...
    $.ajax({url: window.twsparql,
	    data: {"year": year, "limit": window.pageLimit, "offset": offset,
		   "query": "person-publications-paginate.rq",
		   "uri": window.instanceURI,
		   "xslt": "generate/person-publications-paginate.xsl"},
	    success: response,
	    dataType: "xml",
	    error: response
	   });
...
function paginate() {
    ...
    window.pageOffset = 0;
    window.lastYear = "";
    window.pageHeader = $(".publications-paginate-header");
    window.pageFooter = $(".publications-paginate-footer");
